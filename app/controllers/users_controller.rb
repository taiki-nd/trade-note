class UsersController < ApplicationController
  ...
  def show
    @records = Record.where(user_id: @user.id).includes(:user).order(date: "DESC").order(hour_id: "DESC").order("created_at DESC").limit(4)
  ...
  def record
    @records = Record.where(user_id: @user.id).includes(:user).order(date: "DESC").order(hour_id: "DESC").order("created_at DESC").page(params[:page]).per(16)
  end
